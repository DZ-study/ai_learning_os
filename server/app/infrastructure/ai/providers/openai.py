...
基于 openai SDK 的 AsyncOpenAI 实现。
支持 GPT-4o、GPT-4、GPT-3.5 等 OpenAI 模型。
"""
...
class OpenAIProvider(BaseLLMProvider):
    ...
    def _raise_error(exc: Exception) -> None:
        """将 openai SDK 异常映射为项目统一异常。"""
        msg = str(exc)
        if "timeout" in msg.lower() or "timed out" in msg.lower():
            raise LLMTimeoutError(msg) from exc
        raise LLMApiError(
            f"OpenAI API 调用失败: {msg}", data={"original_error": msg}
        ) from exc
